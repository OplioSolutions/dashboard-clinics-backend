 ...
 */
export function outboundCallbackAuth(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!webhookSecret) {
      ...
      throw new AppError(500, 'Callback authentication not configured')
    }
 ...
 */
export function simpleCallbackAuth(req: Request, res: Response, next: NextFunction) {
  try {
    const expectedToken = process.env.N8N_OUTBOUND_CALLBACK_TOKEN
    if (!expectedToken) {
      throw new AppError(500, 'Callback token not configured')
    }

    const providedToken = req.get('x-callback-token') || req.get('authorization')?.replace('Bearer ', '')
    
    if (!providedToken) {
      throw new AppError(401, 'Missing callback token')
    }

    if (providedToken !== expectedToken) {
      throw new AppError(401, 'Invalid callback token')
    }
