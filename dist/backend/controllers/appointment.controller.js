...
class AppointmentController extends base_1.BaseController {
    constructor() {
        super('appointments');
    }
    async list(req, res) {
    ...
    async create(req, res) {
        return this.handleRequest(req, res, async () => {
            const newAppointment = schemas_1.appointmentCreateSchema.parse({
                ...req.body,
            ...
            if (conflictError)
                throw conflictError;
            if (conflicts?.length) {
                throw new errors_1.AppError(409, 'Time slot not available');
            }
    ...
    async update(req, res) {
        return this.handleRequest(req, res, async () => {
            const { id } = req.params;
            const updates = schemas_1.appointmentUpdateSchema.parse(req.body);
